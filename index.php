...
<script type="text/javascript">
    ...
    function switchworld(n) {
        var map_ids = ['world', 'outland', 'northrend'];
        map_ids.forEach((map, i) => {
            document.getElementById(map).style.visibility = (i === n) ? 'visible' : 'hidden';
        });

        var map_links = document.querySelectorAll('#server_info span');
        map_links.forEach((link, i) => {
            if (i === n) {
    ...
    function show(players) {
        ...
        players.forEach(player => {
            ...
            if (maps_for_points.indexOf(mapId) > -1) {
                var pos = get_player_position(player.position_x, player.position_y, player.map);
                var pointHTML = '<div class="player-dot class-' + player.class + '" style="left:' + pos.x + 'px; top:' + pos.y + 'px;" onmousemove="tip(event, \'' + player.name + '\');" onmouseout="h_tip();"></div>';
